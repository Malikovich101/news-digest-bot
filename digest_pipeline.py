...
class DigestPipeline:
    """Production orchestration with explicit dependencies and one state object."""
    ...
    def remember_delivered_news(self, state, posts, delivered_at):
        history = {item["id"]: item for item in state.get("recent_news", []) if isinstance(item, dict) and item.get("id")}
        for post in posts:
            history[post["id"]] = {
                "id": post["id"],
                "date": post["date"],
    ...
    def collect_posts(self, client, channels, state, now, replay_hours=0):
        ...
        legacy_cutoff = self.parse_datetime(state.get("last_run"), now - timedelta(hours=digest.FIRST_RUN_LOOKBACK_HOURS))
        ...
        return posts, channel_updates, failed_channels
    ...
    def _history_candidates(self, current_posts, history):
        def tokens(text):
            ...
            return {word[:5] if len(word) > 5 else word for word in words if word not in digest.COMMON_WORDS}
    ...
    def cross_run_deduplicate(self, client, posts, history):
        ...
        for batch in digest.make_ai_batches(posts, digest.MAX_PREVIEW_CHARS):
            ...
            for start in range(0, len(candidates), digest.RECENT_NEWS_HISTORY_BATCH):
                response = digest.generate_json(client, digest.recent_news_prompt(batch, candidates[start:start + digest.RECENT_NEWS_HISTORY_BATCH]))
                dropped.update(digest.recent_news_repeat_ids(response, {post["id"] for post in batch}))
    ...
    def semantic_deduplicate(self, client, posts):
        ...
        for batch in digest.make_ai_batches(posts, digest.MAX_PREVIEW_CHARS, overlap=20):
            if len(batch) < 2:
                continue
            payload = [{"id": p["id"], "date": p["date"], "text": self.normalize_text(p["text"])[:digest.MAX_PREVIEW_CHARS]} for p in batch]
            prompt = f"""Ты определяешь только смысловые дубли новостей Telegram. Найди только публикации об одном и том же конкретном событии. Не объединяй похожие темы, новые факты и новые этапы истории. Если есть сомнение — не удаляй. Для одной группы событий оставляй самую РАННЮЮ публикацию по date, более поздние удаляй. Верни JSON: {{\"groups\":[{{\"ids\":[\"id1\",\"id2\"]}}]}}. Данные: {json.dumps(payload, ensure_ascii=False)}"""
    ...
    def send_telegram(self, token, chat_id, text, state, posts):
        ...
        for index, chunk in enumerate(chunks):
            ...
            delivered_ids = [p["id"] for p in posts if p.get("url") and p["url"] in chunk]
            state["delivered_ids"] = list(dict.fromkeys(list(state.get("delivered_ids", [])) + delivered_ids))[-digest.MAX_DELIVERED_IDS:]
            self.save_state(state)

        state["delivered_ids"] = list(dict.fromkeys(list(state.get("delivered_ids", [])) + [p["id"] for p in posts]))[-digest.MAX_DELIVERED_IDS:]
        self.save_state(state)
    ...
    def run(self):
        ...
        if posts:
            try:
                ...
                if replay_hours == 0:
                    posts, cross_run_duplicates = self.cross_run_deduplicate(client, posts, state.get("recent_news", []))
        ...
        digest_text = digest.format_digest(posts, stats, semantic_duplicates, confirmed_ads, ai_unavailable, cross_run_duplicates)
        ...
        print(f"Delivered {len(posts)} original posts from {len(collected)} collected (semantic_duplicates={semantic_duplicates}, cross_run_duplicates={cross_run_duplicates}, confirmed_ads={confirmed_ads}, replay_hours={replay_hours})")
