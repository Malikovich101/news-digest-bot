...
class DigestPipelineTests(unittest.TestCase):
    def test_telegram_chunks_are_rate_limited(self):
        ...
        with patch("digest_pipeline.requests.post", return_value=Response()), patch("digest_pipeline.time.sleep") as sleep, patch.object(pipeline, "save_state"):
    ...
    def test_semantic_duplicate_keeps_earliest_publication(self):
        ...
        with patch("digest_pipeline.generate_json", return_value={"groups": [{"keep": "@early:1", "duplicates": ["@late:2"]}]}):
...
class LongTermMemoryTests(unittest.TestCase):
    def test_recent_news_prunes_entries_older_than_72_hours(self):
        old_time = digest_pipeline.utc_now() - timedelta(hours=80)
...
class TruncatePostTests(unittest.TestCase):
    def test_legacy_truncate_helper_is_now_a_noop(self):
        text = "A" * 5000
