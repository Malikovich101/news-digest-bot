...
class WatermarkRegressionTests(unittest.TestCase):
    def test_failed_channel_does_not_advance_watermark(self):
        ...
        posts, next_state, failed = recent_memory_runtime.collect_posts(
            Client(), ["@ok", "@broken"], state, now
        )

        self.assertEqual(posts, [])
        self.assertEqual(failed, ["@broken"])
        self.assertEqual(
            next_state["channels"]["@ok"]["last_checked_at"],
            now.isoformat(),
        )
        self.assertEqual(
            next_state["channels"]["@broken"]["last_checked_at"],
            state["channels"]["@broken"]["last_checked_at"],
        )
