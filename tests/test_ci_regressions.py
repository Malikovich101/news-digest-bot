...
class CIRegressionTests(unittest.TestCase):
    def test_recent_news_prune_uses_current_72_hour_window(self):
        ...
        result = digest.prune_state(digest.migrate_state({"recent_news": recent_news}), now)
        self.assertEqual([item["id"] for item in result["recent_news"]], ["@recent:1"])
    ...
    def test_failed_collection_keeps_channel_out_of_updates(self):
        pipeline = digest.DigestPipeline()
        now = digest.utc_now()
